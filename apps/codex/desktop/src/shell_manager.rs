...
use codex_plugin_host::HostSnapshot;
use dioxus::prelude::*;
...
fn validate_rendered_add_fn(output_path: &Path, content: &str) -> io::Result<()> {
    ...
    // The visual manager owns ~/.add_fn, so every generated version must be parseable
    // by both shells before the read-only target is replaced.
...
fn run_chflags(flag: &str, path: &Path) -> io::Result<()> {
    ...
    if status.success() {
        ...
        Err(io::Error::other(format!(
            "chflags {flag} {} failed with {status}",
            path.display()
        )))
    }
}
...
mod tests {
    ...
    fn write_readonly_atomic_updates_readonly_file() {
        let temp_dir =
            std::env::temp_dir().join(format!("codex-shell-manager-{}", timestamp_millis()));
