...
pub fn DownloadStationScene() -> Element {
    ...
    let records = match records_resource.read().as_ref() {
        ...
        Some(Err(err)) => {
            return rsx! {
                ContentHeader { title: "下载站".to_string(), subtitle: "统一承接下载记录、桌面研究产物和 recent outputs。".to_string() }
    ...
    let rows: Vec<FileRecord> = records
        ...
        .cloned()
        .collect();
    let downloads_count = records
        .iter()
        .filter(|item| item.scope == "downloads")
        .count();

    rsx! {
        ContentHeader { title: "下载站".to_string(), subtitle: "Rust 化的本地下载站，先承接 Downloads 与安装包归档入口。".to_string() }
...
fn scan_directory(
    ...
    Ok(entries
        ...
        .map(|(path, metadata, modified)| {
            ...
            let status = if is_yesterday(modified) {
                "昨天更新"
            } else {
                "已发现"
            };
...
fn looks_like_research_artifact(name: &str) -> bool {
    ...
    let suffixes = [
        ".md", ".txt", ".json", ".csv", ".pdf", ".html", ".docx", ".xlsx",
    ];
...
fn infer_kind(path: &Path, metadata: &fs::Metadata) -> String {
        ...
        .as_deref()
    {
        ...
        Some("docx") => "Word".to_string(),
        Some("xlsx") => "Spreadsheet".to_string(),
        Some("zip") => "Archive".to_string(),
        Some("tar") | Some("gz") | Some("tgz") => "Archive".to_string(),
...
#[cfg(not(target_arch = "wasm32"))]
fn is_yesterday(modified: SystemTime) -> bool {
    let dt: DateTime<Local> = modified.into();
    let now = Local::now();
    dt.date_naive() == (now - Duration::days(1)).date_naive()
}
