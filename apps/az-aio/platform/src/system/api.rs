...
//! 操作路径来自同一套 [`SystemOperation`](crate::system::catalog::SystemOperation)
...
use crate::core::api_error::{ApiError, ApiQuery, ApiResponse, ok_json};
...
async fn page_handler(ApiQuery(query): ApiQuery<PageQuery>) -> Json<Option<SystemPageView>> {
    let route = query.route.unwrap_or_else(|| {
        crate::system::catalog::SYSTEM_DEFAULT_ROUTE.to_string()
    });
...
fn inject_contract_payload(
    ...
    Err(system_error_response(anyhow!("system operation payload must be object")))
}
...
fn payload_from_body(headers: &HeaderMap, body: &Bytes) -> Result<Value, Response> {
    ...
    if content_type.contains("application/json") {
        return serde_json::from_slice(body)
            .map_err(|error| system_error_response(anyhow!("invalid system operation json: {error}")));
    }
...
fn parse_form_body(body: &Bytes) -> Result<Value, Response> {
    let body = std::str::from_utf8(body)
        .map_err(|error| system_error_response(anyhow!("invalid system operation form: {error}")))?;
    ...
    for pair in body.split('&') {
        ...
        let key = urlencoding::decode(raw_key)
            .map_err(|error| system_error_response(anyhow!("invalid system operation form key: {error}")))?;
        let value = urlencoding::decode(raw_value)
            .map_err(|error| system_error_response(anyhow!("invalid system operation form value: {error}")))?;
...
fn operation_for_http(
    method: &str,
    path: &str,
) -> Result<(crate::system::catalog::SystemPage, crate::system::catalog::SystemOperation), Response>
{
...
mod tests {
    ...
    fn form_payload_becomes_operation_object() {
        ...
        assert_eq!(payload.ok().and_then(|value| value["note"].as_str().map(str::to_string)), Some("执行".to_string()));
    }
