...
//!
//! Protocol messages and shared state are lifted from the standalone
//! `az-drive-app` WebSocket endpoint and adapted to the az-aio worker
//! lifecycle.

use anyhow::Context;
use axum::extract::ws::{Message, WebSocket};
use az_crdt::document::LineCrdtDocument;
use az_drive_core::api::{EntryKey, RelativePath, RootAlias, content_hash as drive_content_hash, object_key_for_hash};
...
impl DriveSyncState {
    ...
    async fn save_doc(
        ...
        let hash = drive_content_hash(snapshot.as_bytes());
