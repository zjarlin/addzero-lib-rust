...
const AZ_AIO_DATABASE_NAME: &str = "az-aio";
const STORE_KEY: &str = "default";
const STORE_TABLE: &str = "sys_shell_manager_store";
const AZ_AIO_ENV_FILE: &str = ".config/addzero/az-aio/az-aio.env";
...
pub(crate) fn load_shell_manager_store(json_path: &Path) -> io::Result<ShellManagerStore> {
    let Some(database_url) = az_aio_database_url() else {
...
async fn load_pg_store_or_seed(
    ...
    let pool = connect_az_aio_pool(database_url).await?;
...
async fn connect_az_aio_pool(database_url: &str) -> io::Result<PgPool> {
...
fn az_aio_database_url() -> Option<String> {
    env_database_url("AZ_AIO_DATABASE_URL")
        .or_else(read_az_aio_env_database_url)
        .filter(|value| is_postgres_url(value))
        .and_then(|value| database_url_for_database(&value, AZ_AIO_DATABASE_NAME).ok())
}
...
fn read_az_aio_env_database_url() -> Option<String> {
    let path = home_dir()?.join(AZ_AIO_ENV_FILE);
    let content = fs::read_to_string(path).ok()?;
    let values = parse_env_pairs(&content);
    values
        .get("AZ_AIO_DATABASE_URL")
...
mod tests {
    ...
    #[test]
    fn database_url_for_database_targets_az_aio() {
        let url = database_url_for_database(
            "postgresql://postgres:secret@127.0.0.1:15432/msc_aio?sslmode=disable",
            AZ_AIO_DATABASE_NAME,
        )
        .expect("rewrite database url");

        assert_eq!(
            url,
            "postgresql://postgres:secret@127.0.0.1:15432/az-aio?sslmode=disable"
        );
    }

    #[test]
    fn quote_pg_identifier_preserves_hyphenated_database_name() {
        assert_eq!(quote_pg_identifier("az-aio"), "\"az-aio\"");
    }

    #[test]
    fn parse_env_pairs_reads_quoted_database_url() {
        let values =
            parse_env_pairs("AZ_AIO_DATABASE_URL='postgresql://postgres:secret@localhost/aio'\n");

        assert_eq!(
            values.get("AZ_AIO_DATABASE_URL").map(String::as_str),
