...
use az_aio_plugin_sync::{
    ...
    SyncAgentRootsConfig, SyncDeviceInfo, SyncEngine, SyncFileSystemObjectStoreConfig,
    SyncObjectManifest, SyncObjectStoreConfig, SyncPgRepository, SyncRootWatcher, SyncWatchPlanner,
    SyncWireMessage, SyncWsConnection, SyncWsReader, SyncWsWriter, bootstrap_sync_agent,
    build_sync_agent_engine, sync_api_router, sync_model::normalize_home_relative_path,
};
use az_rustfs::{rustfs::create_storage_client, types::S3ClientConfig};
use serde::Serialize;
...
async fn serve(args: ServeArgs) -> Result<()> {
    ...
    let mut state =
        az_aio_plugin_sync::SyncApiState::new(engine).with_optional_auth_token(args.auth_token());
    ...
    let listener = TcpListener::bind(bind)
        .await
        .context("bind sync server failed")?;
    let local_addr = listener
        .local_addr()
        .context("read sync server local address failed")?;
...
async fn agent(args: AgentArgs) -> Result<()> {
    ...
    if let Some(endpoint) = args.endpoint() {
        let connection = SyncWsConnection::connect(&endpoint, args.auth_token().as_deref()).await?;
        ...
        send_existing_binary_manifests(&engine, &mut planner, object_store.as_ref(), &mut writer)
            .await?;
        ...
        tokio::select! {
            event = rx.recv() => {
                let Some(event) = event else {
                    bail!("sync watcher channel failed: sync watcher channel closed");
                };
                planner.push(event);
...
async fn send_local_plan_updates(
    ...
    let Some(object_store) = object_store else {
        bail!(object_store_required_message());
    };
...
async fn send_existing_text_updates(engine: &SyncEngine, writer: &mut SyncWsWriter) -> Result<()> {
    for file in engine.files() {
...
fn collect_existing_binary_paths(engine: &SyncEngine) -> Result<Vec<String>> {
    let mut binary_paths = Vec::new();
    for root in engine.roots() {
        let mut pending = vec![root.local_path.clone()];
        while let Some(dir) = pending.pop() {
            let entries = fs::read_dir(&dir).with_context(|| {
                format!("read sync root directory failed for `{}`", dir.display())
            })?;
            for entry in entries {
                let entry = entry.with_context(|| {
                    format!("read sync root entry failed for `{}`", dir.display())
                })?;
                let path = entry.path();
                let metadata = entry.metadata().with_context(|| {
                    format!(
                        "read sync root entry metadata failed for `{}`",
                        path.display()
                    )
                })?;
                if metadata.is_dir() {
...
fn is_utf8_file(path: &Path) -> Result<bool> {
    let bytes = fs::read(path).with_context(|| {
        format!(
            "read sync file for binary classification failed for `{}`",
            path.display()
        )
    })?;
...
fn remove_local_file_if_exists(path: &Path) -> Result<()> {
    match fs::remove_file(path) {
        Ok(()) => Ok(()),
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => Ok(()),
        Err(error) => Err(error)
            .with_context(|| format!("remove synced file failed for `{}`", path.display())),
    }
}
...
fn root_add(args: RootAddArgs) -> Result<()> {
...
impl SyncCliCommand {
    fn parse(mut args: Vec<String>) -> Result<Self> {
        ...
        match command.as_str() {
            ...
            _ => bail!("unknown sync command `{command}`\n\n{}", usage()),
        }
    }
}

fn parse_root_command(mut args: Vec<String>) -> Result<SyncCliCommand> {
    let Some(command) = args.first().cloned() else {
        bail!("{}", usage());
    };
    args.remove(0);
    match command.as_str() {
        ...
        _ => bail!("unknown sync root command `{command}`\n\n{}", usage()),
    }
}
...
impl ServeArgs {
    ...
    fn agent_config(&self) -> Result<SyncAgentConfig> {
...
impl CommonAgentArgs {
    fn parse(parser: &mut ArgParser) -> Result<Self> {
        let device_name = parser.take_option("--device-name")?;
        let home = parser.take_option("--home")?.map(PathBuf::from);
...
impl ArgParser {
    ...
    fn take_option(&mut self, flag: &str) -> Result<Option<String>> {
        let Some(index) = self.args.iter().position(|value| value == flag) else {
            return Ok(None);
        };
        self.args.remove(index);
    ...
    fn take_repeated_option(&mut self, flag: &str) -> Result<Vec<String>> {
    ...
    fn take_positional(&mut self, label: &'static str) -> Result<String> {
    ...
    fn finish(self) -> Result<()> {
        if self.args.is_empty() {
            ...
            bail!("unknown arguments: {}\n\n{}", self.args.join(" "), usage());
        }
    }
}
...
fn home_relative_input(device: &SyncDeviceInfo, value: &str) -> Result<String> {
...
fn print_json(value: &impl Serialize) -> Result<()> {
    let json = serde_json::to_string_pretty(value).context("JSON output failed")?;
    println!("{json}");
    Ok(())
}
...
impl AgentReport {
    fn from_engine(event: impl Into<String>, engine: &SyncEngine) -> Result<Self> {
        Ok(Self {
            event: event.into(),
...
fn object_store_required_message() -> &'static str {
...
mod tests {
    ...
    use super::{
        ...
        collect_existing_binary_paths, default_alias_for_relative_path, space_id_for_relative_path,
        sync_ws_endpoint,
    };
    ...
    #[test]
    fn selects_space_id_from_longest_matching_sync_root() -> Result<(), Box<dyn std::error::Error>>
    {
