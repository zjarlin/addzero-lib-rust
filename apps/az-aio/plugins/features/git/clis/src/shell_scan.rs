...
impl ShellScan {
    pub(crate) fn shell_entries(&self) -> impl Iterator<Item = &ShellEntryContribution> {
        self.entries.iter().map(|entry| &entry.contribution)
    }
...
pub(crate) fn scan_shell_sources(source_root: &Path, extra_cli_roots: &[PathBuf]) -> ShellScan {
...
fn scan_shell_root(root: &Path) -> ShellScan {
...
pub(crate) fn pending_generated_file(
    scan: &ShellScan,
    source_root: &Path,
    output_path: &Path,
...
pub(crate) fn managed_generated_file(
    scan: &ShellScan,
    source_root: &Path,
    output_path: &Path,
) -> GeneratedFileContribution {
...
mod tests {
    ...
    fn generated_file_metadata_points_to_visual_manager() {
        ...
        assert!(generated.message.contains("可视化命令管理器"));
    }
}
