...
use az_aio_plugin_api::api::{GeneratedFileStatus, NativeAzAioPlugin};
use az_aio_plugin_git_clis::plugin::GitClisPlugin;
use tempfile::TempDir;

#[test]
fn plugin_contributions_scans_entries_without_rewriting_add_fn() {
    ...
    let plugin = GitClisPlugin::new(&root, &output, Vec::new());
    ...
    // managed_generated_file preserves existing content on rewrites
    ...
    assert!(contributions.generated_files[0]
        .message
        .contains("可视化命令管理器"));
}
