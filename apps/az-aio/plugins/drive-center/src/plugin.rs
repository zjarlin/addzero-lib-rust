...
use crate::{
    ...
    page::DriveCenterPage,
    routes::{DriveCenterApiState, drive_center_router},
    store::build_drive_center_module,
};

#[derive(Default)]
pub struct DriveCenterPlugin;

impl NativeAzAioPlugin for DriveCenterPlugin {
    ...
    fn runtime(&self, context: NativePluginContext) -> anyhow::Result<NativePluginRuntime> {
        let _module = build_drive_center_module();
...
register_native_plugin!(DriveCenterPlugin);
