...
fn handle_action(store: &LowcodeStore, action: &Option<String>, route: &str) {
    ...
    match action {
        ...
        "new-field" => {
            ...
            let label = parse_query_param(route, "field_label").unwrap_or_else(|| name.clone());
            let ft = parse_query_param(route, "field_type").unwrap_or_else(|| "String".into());
            let rel_type = parse_query_param(route, "rel_type").filter(|v| !v.is_empty());
            let rel_model_id = parse_query_param(route, "rel_model_id").filter(|v| !v.is_empty());
        ...
        "batch-delete-record" => {
            let model_id = parse_query_param(route, "rec_model").unwrap_or_default();
            let ids = parse_query_param(route, "ids").unwrap_or_default();
            if !model_id.is_empty() && !ids.is_empty() {
                let rec_store = RecordStore::global();
                for id in ids.split(',') {
                    let id = id.trim();
                    if !id.is_empty() {
                        rec_store.delete(&model_id, id);
                    }
                }
            }
        }
        ...
        "edit-screen" => {
            ...
            if !sid.is_empty() {
                ...
                if let (Some(new_layout), Some(new_model_id)) =
        ...
        "new-screen" => {
            ...
            let label = parse_query_param(route, "scr_label").unwrap_or_else(|| name.clone());
            let layout = parse_query_param(route, "scr_layout").unwrap_or_else(|| "Table".into());
