...
pub fn lowcode_router(state: LowcodeApiState) -> Router {
    Router::new()
        ...
        .route(
            "/api/lowcode/screens",
            get(list_screens).post(create_screen),
        )
        .route(
...
) -> Json<Vec<crate::backend::model::MetaModelSummary>> {
...
) -> Json<Vec<crate::backend::model::MetaFieldView>> {
...
async fn update_field(
    ...
    if input.enum_options.is_some() {
        field.enum_options = input.enum_options;
    }
...
async fn list_screens(
    State(s): State<LowcodeApiState>,
) -> Json<Vec<crate::backend::model::AppScreenSummary>> {
...
async fn update_screen(
    State(s): State<LowcodeApiState>,
    Path(id): Path<String>,
    Json(input): Json<UpdateScreenInput>,
) -> Json<serde_json::Value> {
    match s
        .store
        .update_screen(&id, &input.name, &input.label, &input.layout)
        .await
    {
        Ok(Some(_)) => Json(serde_json::json!({ "ok": true })),
...
pub fn record_router() -> axum::Router<LowcodeApiState> {
    axum::Router::new()
        .route(
            "/api/lowcode/records/{model_id}",
            axum::routing::get(list_records).post(create_record),
        )
        .route(
            "/api/lowcode/records/{model_id}/{id}",
            axum::routing::put(update_record).delete(delete_record),
        )
}

async fn list_records(Path(model_id): Path<String>) -> Json<Vec<RecordWithId>> {
...
async fn delete_record(Path((model_id, id)): Path<(String, String)>) -> Json<serde_json::Value> {
    let store = RecordStore::global();
