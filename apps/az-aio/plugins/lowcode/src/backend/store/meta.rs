...
use crate::backend::model::{AppScreen, MetaField, MetaFieldView, MetaModel, MetaModelSummary};
...
impl LowcodeStore {
    ...
    pub async fn list_models(&self) -> anyhow::Result<Vec<MetaModelSummary>> {
        if let Some(ref db) = self.db {
            ...
            for m in models {
                let c = Query::<List<MetaField>>::filter(MetaField::fields().model_id().eq(&m.id))
                    .exec(&mut *db)
                    .await?
                    .len() as i64;
    ...
    pub async fn delete_model(&self, id: &str) -> anyhow::Result<()> {
        if let Some(ref db) = self.db {
            let mut db = db.lock().await;
            AppScreen::filter(crate::backend::model::AppScreen::fields().model_id().eq(id))
                .delete()
                .exec(&mut *db)
                .await?;
    ...
    pub async fn list_fields(&self, model_id: &str) -> anyhow::Result<Vec<MetaFieldView>> {
        if let Some(ref db) = self.db {
            let mut db = db.lock().await;
            let fields =
                Query::<List<MetaField>>::filter(MetaField::fields().model_id().eq(model_id))
                    .exec(&mut *db)
                    .await?;
            ...
            for v in &mut views {
                if let Some(ref rel_id) = v.relation_model_id {
                    if let Ok(Some(m)) = Query::<List<MetaModel>>::filter(
                        MetaModel::fields().id().eq(rel_id.as_str()),
                    )
                    .first()
                    .exec(&mut *db)
                    .await
