...
use crate::backend::model::{AppScreen, AppScreenSummary, MetaModel};
...
impl LowcodeStore {
    ...
    pub async fn list_screens(&self) -> anyhow::Result<Vec<AppScreenSummary>> {
        if let Some(ref db) = self.db {
            ...
            for s in screens {
                let mn = Query::<List<MetaModel>>::filter(MetaModel::fields().id().eq(&s.model_id))
                    .first()
                    .exec(&mut *db)
                    .await?
                    .map(|m| m.name)
