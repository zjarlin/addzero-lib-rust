...
mod tests {
    ...
    #[test]
    fn cell_reference_roundtrip_works() {
        assert_eq!(encode_cell_reference(0, 0), "A1");
        assert_eq!(encode_cell_reference(2, 27), "AB3");
        assert_eq!(parse_cell_reference("AB3").unwrap(), (2, 27));
    }
    ...
    #[test]
    fn shared_strings_are_read() {
        let xml = concat!(
            r#"<?xml version="1.0" encoding="UTF-8" standalone="yes"?>"#,
            r#"<sst xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">"#,
            r#"<si><t>hello</t></si>"#,
            r#"<si><r><t>rich</t></r><r><t> text</t></r></si>"#,
            r#"</sst>"#
        );
        assert_eq!(
            parse_shared_strings(xml).unwrap(),
            vec!["hello".to_owned(), "rich text".to_owned()]
        );
    }
}
