...
mod tests {
    ...
    #[test]
    fn local_tilde_paths_expand_to_home_directory() {
        let expanded = expand_local_path(Path::new("~/demo.txt"));
        let home = std::env::var("HOME").expect("HOME should be available in tests");
        assert_eq!(expanded, PathBuf::from(home).join("demo.txt"));
    }

    #[test]
    fn remote_path_helpers_handle_root_and_nested_paths() {
        assert_eq!(normalize_remote_path(" /tmp/demo/ "), "/tmp/demo");
        assert_eq!(normalize_remote_path("/"), "/");
        assert_eq!(append_remote_path("/tmp", "file.txt"), "/tmp/file.txt");
