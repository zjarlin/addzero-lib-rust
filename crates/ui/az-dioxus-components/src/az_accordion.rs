...
pub fn AzAccordion(
    ...
    let root_class = compose_class("az-accordion lowcode-accordion", &class, &[]);
    let summary_class = compose_class(
        "az-accordion__summary lowcode-accordion__summary",
        &summary_class,
        &[],
    );
    let body_class = compose_class("az-accordion__body lowcode-accordion__body", &body_class, &[]);
