...
impl GrammarSearchField {
    /// 创建一个可展示在 `GrammarSearchInput` 中的过滤维度提示。
...
#[component]
pub fn GrammarSearchInput(
    value: String,
    oninput: EventHandler<String>,
    #[props(default, into)] class: String,
    #[props(default, into)] placeholder: String,
    #[props(default)] fields: Vec<GrammarSearchField>,
) -> Element {
    let root_class = compose_class("grammar-search", &class, &[]);
    ...
    rsx! {
        ...
        div { class: root_class,
            ...
            if !filters.is_empty() || !terms.is_empty() {
                div { class: "grammar-search__tokens",
                    for term in terms {
                        span { class: "grammar-search__token grammar-search__token--term",
                            "{term.value}"
                        }
                    }
                    for filter in filters {
                        for filter_value in filter.values {
                            span { class: "grammar-search__token",
                                span { class: "grammar-search__token-key", "{filter.key}" }
                                span { class: "grammar-search__token-separator", ":" }
                                span { class: "grammar-search__token-value", "{filter_value}" }
                            }
                        }
                    }
                }
            ...
            if !fields.is_empty() {
                div { class: "grammar-search__fields",
                    for field in fields {
                        span { class: "grammar-search__field",
                            span { class: "grammar-search__field-key", "{field.key}" }
                            span { class: "grammar-search__field-label", "{field.label}" }
                        }
                    }
                }
            }
