...
use az_face_detection::logic_face_detection::model::{
    FaceDetectionOptions, MODEL_CODE as FACE_DETECTION_MODEL_CODE,
};
use az_face_recognition::logic_face_recognition::model::{
...
use az_qr_code_recognition::logic_qr_code_recognition::model::ALGORITHM_CODE as QR_CODE_RECOGNITION_CODE;
use az_safety_helmet_detection::logic_safety_helmet_detection::model::{
    ALGORITHM_CODE as SAFETY_HELMET_DETECTION_CODE, SAFETY_HELMET_DETECTION_PPE_YOLO11S,
};
use az_vehicle_detection::logic_vehicle_detection::model::{
...
fn person_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-person-detection/tests/fixtures/input")
...
fn person_model_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-person-detection/resources/models")
...
fn face_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-face-detection/tests/fixtures/input")
            .join("face.jpg"),
    )
    .expect("人脸检测测试图片必须存在")
}
...
fn safety_helmet_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-safety-helmet-detection/tests/fixtures/input")
...
fn safety_helmet_model_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-safety-helmet-detection/resources/models")
...
fn vehicle_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-vehicle-detection/tests/fixtures/input")
            .join("person_vehicle.jpg"),
    )
    .expect("车辆检测测试图片必须存在")
}
...
fn vehicle_model_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-vehicle-detection/resources/models")
            .join("coco_ssd_mobilenet_v1_10.onnx"),
    )
    .expect("车辆检测模型必须存在")
}
...
fn flame_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-flame-detection/tests/fixtures/input")
            .join("flame.jpg"),
    )
    .expect("火焰检测测试图片必须存在")
}
...
fn face_recognition_model_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-face-recognition/resources/models")
            .join("face_recognition_arcface_resnet100_int8.onnx"),
    )
    .expect("人脸识别模型必须存在")
}
...
fn ocr_text_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-ocr-text-recognition/tests/fixtures/input")
            .join("ocr_text.jpg"),
    )
    .expect("OCR 测试图片必须存在")
}
...
fn ocr_text_detection_model_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-ocr-text-recognition/resources/models")
...
fn qr_code_fixture_path() -> PathBuf {
    std::fs::canonicalize(
        workspace_root()
            .join("crates/algorithm/az-qr-code-recognition/tests/fixtures/input")
            .join("qr_code.png"),
    )
    .expect("二维码测试图片必须存在")
}
...
-> AlgorithmVideoPipelineResult<()> {
    ...
    dbg!(&face_recognition_model_path());
    ...
    let mut face_recognition_algorithm = OnnxRawImageVideoAlgorithm::new(
        ...
        output_dir.join("face_recognition_raw_frames"),
    )?;
    ...
    let mut bindings = [
        ...
        VideoAlgorithmBinding {
            algorithm: &mut face_recognition_algorithm,
            schedule: VideoAlgorithmSchedule::EveryFrame,
        },
        VideoAlgorithmBinding {
    ...
    assert_existing_file(&run.files.frame_results_jsonl);
    assert_existing_file(&run.files.summary_json);
    // 关键断言：一帧输入流上，8 个算法各执行一次，证明可以在实时视频帧上叠加。
    assert_eq!(
        run.frame_results
            .iter()
