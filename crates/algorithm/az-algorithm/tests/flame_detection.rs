...
use az_algorithm::components::flame_detection::assist::{
...
fn fixture_path(file_name: &str) -> PathBuf {
    std::fs::canonicalize(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/flame_detection/input")
            .join(file_name),
    )
    .expect("测试输入图片必须存在")
}
...
fn model_path() -> PathBuf {
    std::fs::canonicalize(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("resources/flame_detection/models")
...
fn flame_detection_should_run_real_image_and_write_outputs() -> anyhow::Result<()> {
    // 输入图片：crates/algorithm/az-algorithm/tests/fixtures/flame_detection/input/flame.jpg
