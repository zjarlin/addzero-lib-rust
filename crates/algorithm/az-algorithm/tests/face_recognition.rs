...
use az_algorithm::components::face_recognition::assist::run_face_recognition_from_path_with_output;
...
fn fixture_path(file_name: &str) -> PathBuf {
    std::fs::canonicalize(
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("tests/fixtures/face_recognition/input")
            .join(file_name),
    )
    .expect("测试输入图片必须存在")
}
...
#[test]
fn face_recognition_should_run_real_image_and_write_outputs() -> anyhow::Result<()> {
    // 输入图片：
    // crates/algorithm/az-algorithm/tests/fixtures/face_recognition/input/face.jpg
    //
    // 模型：
    // crates/algorithm/az-algorithm/resources/face_recognition/models/face_recognition_arcface_resnet100_int8.onnx
    //
    // 输出：
