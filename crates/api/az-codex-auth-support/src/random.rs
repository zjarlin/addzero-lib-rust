...
pub(crate) fn random_local_part(length: usize) -> CodexAuthSupportResult<String> {
    random_string(length.max(3), LOCAL_PART)
}

pub(crate) fn random_password(length: usize) -> CodexAuthSupportResult<String> {
    ...
    String::from_utf8(chars).map_err(|_| {
        CodexAuthSupportError::InvalidResponse("generated password was not UTF-8".to_owned())
    })
}

pub(crate) fn random_bytes<const N: usize>() -> CodexAuthSupportResult<[u8; N]> {
    let rng = SystemRandom::new();
    let mut bytes = [0u8; N];
    rng.fill(&mut bytes)
        .map_err(|_| CodexAuthSupportError::Crypto)?;
    Ok(bytes)
}

fn random_string(length: usize, alphabet: &[u8]) -> CodexAuthSupportResult<String> {
    let mut chars = Vec::with_capacity(length);
    for _ in 0..length {
        chars.push(random_char(alphabet)?);
    }
    String::from_utf8(chars).map_err(|_| {
        CodexAuthSupportError::InvalidResponse("generated string was not UTF-8".to_owned())
    })
}

fn random_char(alphabet: &[u8]) -> CodexAuthSupportResult<u8> {
    let bytes = random_bytes::<1>()?;
    let index = usize::from(bytes[0]) % alphabet.len();
    Ok(alphabet[index])
}

fn shuffle(values: &mut [u8]) -> CodexAuthSupportResult<()> {
    if values.len() < 2 {
        return Ok(());
    }
