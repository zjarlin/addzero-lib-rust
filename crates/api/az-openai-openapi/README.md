...
It fetches the remote OpenAPI spec and writes real Rust source files under `src/generated`, so DTO fields and REST traits stay visible to IDEs and code review.
...
Module collection follows the addzero automod convention: `src/lib.rs` stays as the crate entrypoint using `automod::dir!`, `src/generated.rs` collects generated modules, and `src/api.rs`, `src/models.rs`, `src/paths.rs`, and `src/bodies.rs` keep stable public module paths.
