...
//! ## 线性规划
//!
//! - [`LinearProgrammingProblem`] — 定义目标函数、约束条件，通过活动集枚举法求解
//! - [`solve_linear_programming()`] — 面向运输/配置问题的便捷求解接口
//! - [`solve_linear_programming_with_prices()`] — 带价格矩阵的扩展求解接口
//!
//! ## 类型概览
//!
//! - [`Matrix<T>`] — `Vec<Vec<T>>` 的类型别名
//! - 公开函数统一返回 `anyhow::Result<T>`，并在矩阵校验、线性规划求解失败时携带可读上下文
//!
//! # 限制
//!
//! 线性规划采用穷举活动集法，变量和约束数量较大时会返回错误。
//! 默认最大枚举组合数为 500,000。
...
impl LinearProgrammingProblem {
    ...
    pub fn solve(&self) -> Result<LinearProgrammingSolution> {
        solve_problem(self)
    }
}
...
pub fn transpose_matrix(matrix: &[Vec<f64>]) -> Result<Matrix<f64>> {
...
pub fn get_matrix_element<T>(matrix: &[Vec<T>], row: usize, col: usize) -> Result<&T> {
    validate_rectangular(matrix)?;
    let Some(value) = matrix.get(row).and_then(|r| r.get(col)) else {
        bail!("matrix element ({row}, {col}) is out of bounds");
    };
    Ok(value)
}
...
pub fn set_matrix_elements<T: Clone>(
    ...
    for update in updates {
        let Some(cell) = result
            ...
            bail!(
                "matrix element ({}, {}) is out of bounds",
                update.row,
                update.col
            );
        };
...
pub fn generate_vector(rows: usize, cols: usize, indices_1_based: &[usize]) -> Result<Vec<f64>> {
    let len = rows * cols;
    let mut vector = vec![0.0; len];
    for index in indices_1_based {
        if *index == 0 || *index > len {
            bail!("index {} is out of bounds for vector length {len}", *index);
        }
        vector[index - 1] = 1.0;
    }
    Ok(vector)
...
pub fn generate_index_groups(total: usize, group_size: usize) -> Result<Vec<Vec<usize>>> {
    if group_size == 0 {
        bail!("group size must be greater than zero");
    }
...
pub fn set_interval_elements(base_row: &[i32], group_size: usize) -> Result<Vec<Vec<i32>>> {
...
) -> Result<LinearProgrammingSolution> {
...
fn solve_problem(problem: &LinearProgrammingProblem) -> Result<LinearProgrammingSolution> {
    let variable_count = problem.variable_count();
    if variable_count == 0 {
        bail!("problem has no variables");
    }

    for constraint in &problem.constraints {
        if constraint.coefficients.len() != variable_count {
            bail!(
                "constraint dimension mismatch: expected {variable_count}, got {}",
    ...
    if combinations > DEFAULT_MAX_COMBINATIONS {
        bail!("too many active-set combinations to enumerate: {combinations}");
    }
...
fn validate_transport_dimensions(
    ...
    if capacities.iter().any(|row| row.len() != zbs.len()) {
        bail!("capacity rows must all have the same length as zbs");
    }
    if price_matrix.len() != line_configurations.len() {
        bail!("price matrix row count must equal line configuration count");
    }
    if price_matrix.iter().any(|row| row.len() != ship_types) {
        bail!("price matrix column count must equal capacity row count");
    }
    Ok(())
}
