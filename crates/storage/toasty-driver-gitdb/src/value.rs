...
pub(crate) fn to_json_value(value: &Value) -> Result<JsonValue, GitDbDriverError> {
    match value {
        ...
        Value::F32(v) => serde_json::Number::from_f64((*v).into())
            ...
            .ok_or_else(|| GitDbDriverError::UnsupportedValue("non-finite f32".into())),
        Value::F64(v) => serde_json::Number::from_f64(*v)
            .map(JsonValue::Number)
            .ok_or_else(|| GitDbDriverError::UnsupportedValue("non-finite f64".into())),
        ...
        other => Err(GitDbDriverError::UnsupportedValue(format!("{other:?}"))),
    }
}
...
pub(crate) fn from_json_value(
    ...
    match (ty, value) {
        ...
        (stmt::Type::Uuid, JsonValue::String(v)) => v
            .parse()
            .map(Value::Uuid)
            .map_err(|_| GitDbDriverError::InvalidResult(format!("invalid uuid: {v}"))),
        (stmt::Type::I8, JsonValue::Number(n)) => n
            .as_i64()
            .and_then(|v| i8::try_from(v).ok())
            .map(Value::I8)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected i8".into())),
        (stmt::Type::I16, JsonValue::Number(n)) => n
            .as_i64()
            .and_then(|v| i16::try_from(v).ok())
            .map(Value::I16)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected i16".into())),
        (stmt::Type::I32, JsonValue::Number(n)) => n
            .as_i64()
            .and_then(|v| i32::try_from(v).ok())
            .map(Value::I32)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected i32".into())),
        (stmt::Type::I64, JsonValue::Number(n)) => n
            .as_i64()
            .map(Value::I64)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected i64".into())),
        (stmt::Type::U8, JsonValue::Number(n)) => n
            .as_u64()
            .and_then(|v| u8::try_from(v).ok())
            .map(Value::U8)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected u8".into())),
        (stmt::Type::U16, JsonValue::Number(n)) => n
            .as_u64()
            .and_then(|v| u16::try_from(v).ok())
            .map(Value::U16)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected u16".into())),
        (stmt::Type::U32, JsonValue::Number(n)) => n
            .as_u64()
            .and_then(|v| u32::try_from(v).ok())
            .map(Value::U32)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected u32".into())),
        (stmt::Type::U64, JsonValue::Number(n)) => n
            .as_u64()
            .map(Value::U64)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected u64".into())),
        (stmt::Type::F32, JsonValue::Number(n)) => n
            .as_f64()
            .map(|v| Value::F32(v as f32))
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected f32".into())),
        (stmt::Type::F64, JsonValue::Number(n)) => n
            .as_f64()
            .map(Value::F64)
            .ok_or_else(|| GitDbDriverError::InvalidResult("expected f64".into())),
        (stmt::Type::Bytes, JsonValue::Array(items)) => items
            ...
            .map(|item| match item {
                JsonValue::Number(n) => n
                    .as_u64()
                    .and_then(|v| u8::try_from(v).ok())
                    .ok_or_else(|| GitDbDriverError::InvalidResult("expected byte".into())),
                _ => Err(GitDbDriverError::InvalidResult(
                    "expected numeric byte array".into(),
                )),
            })
            .collect::<Result<Vec<_>, _>>()
            .map(Value::Bytes),
        ...
        (stmt::Type::Record(field_tys), JsonValue::Array(items)) => {
            if items.len() != field_tys.len() {
                return Err(GitDbDriverError::InvalidResult(format!(
        ...
        (ty, value) => Err(GitDbDriverError::InvalidResult(format!(
            "cannot decode {value:?} as {ty:?}"
        ))),
    }
}
