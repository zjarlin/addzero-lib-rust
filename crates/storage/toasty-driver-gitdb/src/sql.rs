...
use crate::value::to_json_value;
use toasty_core::driver::operation::TypedValue;

pub(crate) fn inline_indexed_params(
    sql: &str,
    params: &[TypedValue],
) -> Result<String, GitDbDriverError> {
    let literals = params
        .iter()
    ...
    while let Some(ch) = chars.next() {
        ...
        if !in_single_quote && ch == '?' {
            ...
            let index = digits.parse::<usize>().map_err(|_| {
                GitDbDriverError::InvalidResult(format!("invalid placeholder ?{digits}"))
            })?;
            let literal = literals.get(index.saturating_sub(1)).ok_or_else(|| {
                GitDbDriverError::InvalidResult(format!(
                    "missing parameter for placeholder ?{index}"
                ))
            })?;
...
fn param_to_sql_literal(param: &TypedValue) -> Result<String, GitDbDriverError> {
