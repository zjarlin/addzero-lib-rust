...
use crate::destination::destination_key;
...
async fn get_object(
    ...
    let version = state
        ...
        .await?
        .with_context(|| format!("not found: {}", entry.key.remote_path()))?;
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&format!("\"{}\"", version.content_hash))
            .with_context(|| "invalid header: etag")?,
    );
...
async fn copy_object(
    ...
    let source_entry = state
        ...
        .get_entry(&source)
        .await?
        .with_context(|| format!("not found: {}", source.remote_path()))?;
...
async fn lock_object(
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&format!("<{}>", lock.token))
            .with_context(|| format!("invalid header: {LOCK_TOKEN_HEADER}"))?,
    );
    Ok((
        StatusCode::OK,
        headers,
...
async fn unlock_object(
    ...
    let token = header_or(headers, LOCK_TOKEN_HEADER, "")?
        ...
        .to_owned();
    if token.is_empty() {
        bail!("invalid header: {LOCK_TOKEN_HEADER}");
    }
...
async fn require_entry(
    ...
    state
        ...
        .with_context(|| format!("not found: {}", key.remote_path()))
}

pub(crate) fn entry_key(space: &str, root: &str, relative: &str) -> Result<EntryKey> {
    Ok(EntryKey::new(
        space.to_owned(),
...
pub(crate) fn header_or(headers: &HeaderMap, name: &str, default: &str) -> Result<String> {
...
fn error_response(error: anyhow::Error) -> Response {
    ...
    let status = if message.starts_with("not found:")
        || error_chain_starts_with(&error, "drive entry was not found:")
        || error_chain_starts_with(&error, "drive object was not found:")
    {
        StatusCode::NOT_FOUND
    } else if error_chain_starts_with(&error, "drive entry is locked by `") {
        StatusCode::LOCKED
    } else if message.starts_with("invalid header:") {
...
fn error_chain_starts_with(error: &anyhow::Error, prefix: &str) -> bool {
    error
        .chain()
        .any(|source| source.to_string().starts_with(prefix))
}
