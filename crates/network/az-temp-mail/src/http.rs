...
impl HttpApiClient {
    pub(crate) fn new(config: ApiConfig) -> TempMailResult<Self> {
        config.validate()?;
        let base_url = Url::parse(&config.base_url)
            .map_err(|_| TempMailError::InvalidBaseUrl(config.base_url.clone()))?;
    ...
    pub(crate) fn get(&self, path: &str) -> TempMailResult<RequestBuilder> {
    ...
    pub(crate) fn delete(&self, path: &str) -> TempMailResult<RequestBuilder> {
    ...
    pub(crate) fn read_json<T: DeserializeOwned>(response: Response) -> TempMailResult<T> {
        let response = Self::ensure_success(response)?;
        let bytes = response.bytes()?;
        Ok(serde_json::from_slice(bytes.as_ref())?)
    }

    fn ensure_success(response: Response) -> TempMailResult<Response> {
    ...
    fn join_url(&self, path: &str) -> TempMailResult<Url> {
        self.base_url
            .join(path)
            .map_err(|_| TempMailError::InvalidPath(path.to_owned()))
    }
}

fn build_header_map(headers: &BTreeMap<String, String>) -> TempMailResult<HeaderMap> {
    let mut header_map = HeaderMap::new();
    for (name, value) in headers {
        let header_name = HeaderName::from_bytes(name.as_bytes()).map_err(|source| {
            TempMailError::InvalidHeaderName {
                name: name.clone(),
                source,
            }
        })?;
        let header_value =
            HeaderValue::from_str(value).map_err(|source| TempMailError::InvalidHeaderValue {
                name: name.clone(),
                source,
            })?;
