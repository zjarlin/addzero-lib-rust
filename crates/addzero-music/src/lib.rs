...
impl MusicSearchApi {
    ...
    pub fn search_by_lyric(&self, lyric_fragment: impl Into<String>) -> MusicResult<Vec<MusicSong>> {
        Ok(self
            .search(
...
impl HttpApiClient {
    fn new(config: ApiConfig) -> MusicResult<Self> {
        config.validate()?;
        let base_url =
            Url::parse(&config.base_url).map_err(|_| MusicError::InvalidBaseUrl(config.base_url))?;
    ...
    fn read_json<T: DeserializeOwned>(&self, url: Url, response: Response) -> MusicResult<T> {
        let response = self.ensure_success(&url, response)?;
        let bytes = response.bytes()?;
        Ok(serde_json::from_slice(bytes.as_ref())?)
    }

    fn ensure_success(&self, url: &Url, response: Response) -> MusicResult<Response> {
    ...
    fn build_url(&self, path: &str, query: &[(&str, String)]) -> MusicResult<Url> {
...
fn build_default_headers(headers: &BTreeMap<String, String>) -> MusicResult<HeaderMap> {
...
fn apply_headers(
    ...
    headers: &BTreeMap<String, String>,
) -> MusicResult<reqwest::blocking::RequestBuilder> {
    for (name, value) in headers {
        let header_name = HeaderName::from_bytes(name.as_bytes()).map_err(|source| {
            MusicError::InvalidHeaderName {
                name: name.clone(),
